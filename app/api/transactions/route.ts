...
export async function GET(req: NextRequest) {
  try {
    ...
    const transactions = db.prepare(query).all(...args)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = db.prepare(`
      INSERT INTO transactions (type, amount, description_he, description_en, category_id, date, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(type, amount, description_he || null, description_en || null, category_id || null, date, notes || null)
